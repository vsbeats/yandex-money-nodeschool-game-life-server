class LifeGameVirtualDom {
	...
	constructor(updateInterval = 2000, pointSize = 10, fieldX = 640, fieldY = 480) {
		this.size = {
			point: pointSize,
			fieldX,
			fieldY,
			...
			for (let j = 0; j < sizeY; j++) {
				...
				if (this._checkPoint(i, j)) {
					if (neighborsCount < 2 || neighborsCount > 3) {
						const {user} = this._virtualField[i][j];
						this._delPoint(user, i, j);
					}
				} else {
					if (neighborsCount === 3) {
	...
	_addPoint(user, x, y) {
		this._incrementUserScore(user);
		this._addAffected(user, x, y, true);
	}

	_delPoint(user, x, y) {
		this._decrementUserScore(user);
		this._addAffected(user, x, y, false);
	}

	_findUser(userToFind) {
		return this.players.find(player => userToFind.token === player.token);
	}

	_incrementUserScore(userToFind) {
		const player = this._findUser(userToFind);
		player.population++;
	}

	_decrementUserScore(userToFind) {
		const player = this._findUser(userToFind);
		player.population--;
	}
